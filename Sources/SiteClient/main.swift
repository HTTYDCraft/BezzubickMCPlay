...
func fmtCount(_ val: Double) -> String {
    if val >= 1_000_000 {
        ...
        let s = "\(rounded)"
        if s.hasSuffix(".0") { return String(s.dropLast(2)) + "M" }
        return s + "M"
    }
    if val >= 1_000 {
        let k = val / 1_000
        ...
        let s = "\(rounded)"
        if s.hasSuffix(".0") { return String(s.dropLast(2)) + "K" }
        return s + "K"
    }
    return "\(Int(val))"
}
...
let HOME_STR_RU: [String: String] = [
    ...
    "liveSub": "Обычно стримы по пятницам, 17:00\u{2013}19:00 МСК.",
...
func applyTheme(_ theme: String) {
    let body = jsGlobal("document").body!
...
func setupCalendar() {
    ...
    calYear = now.getFullYear!().integer ?? 2026
    calMonth = now.getMonth!().integer ?? 0
...
func renderCal() {
    ...
    let firstDow = jsGlobal("Date").new!(calYear, calMonth, 1)
    let startDow = ((firstDow.getDay!().integer ?? 0) + 6) % 7
    let daysInMonth = jsGlobal("Date").new!(calYear, calMonth + 1, 0).getDate!().integer ?? 30
    let today = jsGlobal("Date").new!()
    ...
    for d in 1...daysInMonth {
        let dt = jsGlobal("Date").new!(calYear, calMonth, d)
...
func fetchData() async {
    let ts = (jsGlobal("Date").new!()["getTime"]().double ?? 0)
    let win = jsGlobal("window")
    let resp = win.fetch!("/data.json?t=\(Int(ts))")
    ...
    // Streams history
    let ts2 = (jsGlobal("Date").new!()["getTime"]().double ?? 0)
    let resp2Val = try? await win.fetch!("/streams_history.json?t=\(Int(ts2))")
    ...
    for i in 0..<Int(length) {
        ...
        let onEnd = JSClosure { _ in
            ...
            if swipeActive {
                let threshold = 100.0
                if dx > threshold {
                    if !subUrl.isEmpty { jsGlobal("window").open!(subUrl) } else { jsGlobal("window").open!(url) }
                } else if dx < -threshold && platformId == "youtube" {
                    jsGlobal("window").open!(url)
                }
            }
    ...
    _ = devBtn.addEventListener!("click", JSClosure { _ in
        ...
        if !devView.isUndefined {
            ...
            if !upd.isUndefined {
                ...
                let d = zeroPad(now.getDate!().integer!, 2)
...
func initWebGL() {
    canvas = jsGlobal("document").createElement!("canvas")
...
func renderFrame() {
    ...
    let win = jsGlobal("window")
    let dpr = min(win.devicePixelRatio.double ?? 1.0, 2.0)
    let ww = win.innerWidth.double ?? 1, wh = win.innerHeight.double ?? 1
...
func setupMouseTracking() {
    ...
    let fn = JSClosure { args in
        let e = args[0]
        mouseXFrac = (e.clientX.double ?? 0) / (win.innerWidth.double ?? 1)
        mouseYFrac = 1.0 - (e.clientY.double ?? 0) / (win.innerHeight.double ?? 1)
        return .undefined
    }
