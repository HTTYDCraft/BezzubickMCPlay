    ...
    for i in 0..<Int(length) {
        ...
        let onEnd = JSClosure({ _ in
            ...
            if swipeActive {
                let threshold = 100.0
                if dx > threshold {
                    let w = jsGlobal("window")
                    if !subUrl.isEmpty { _ = w.open(subUrl) } else { _ = w.open(url) }
                } else if dx < -threshold && platformId == "youtube" {
                    _ = jsGlobal("window").open(url)
                }
            }
...
func observeGlassState() {
    ...
    _ = obs.new(closure)
    _ = obs.observe(body, ["attributes": true, "attributeFilter": ["class"]] as JSObject)
    let isGlass = body.classList.contains("glass-dark").boolean ?? false || body.classList.contains("glass-light").boolean ?? false
